angular.module('templates-dist', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put("src/features/branding/contact.html",
    "<ul class=contact><li class=contact-item ng-repeat=\"contact in contacts\">{{ contact }}</li></ul>");
  $templateCache.put("src/features/todos/todos.html",
    "<form id=todo-form ng-submit=addCurrentTodo() ng-if=false>Enter new todo here: <input ng-model=newTodo placeholder=\"enter new todo\"></form><ul><li class=todo ng-repeat=\"todo in todos\">{{ todo }}</li></ul>");
}]);
